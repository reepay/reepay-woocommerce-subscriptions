...
class WC_Reepay_Subscription_API {
	...
	public function request( string $endpoint, string $method = 'GET', $params = [] ) {
		...
		if( empty( $key ) ) {
			throw new Exception(
				sprintf(
					__( 'Frisbii Optimize: API key not specified. Specify it in <a href="%s" target="_blank">gateway settings</a>', 'reepay-checkout-gateway' ),
		...
		switch ( $code ) {
			...
			case 5:
				if ( mb_strpos( $body, 'Request rate limit exceeded', 0, 'UTF-8' ) !== false ) {
					...
					if ( $request_retry ) {
						throw new Exception( __( 'Frisbii Optimize: Request rate limit exceeded',
