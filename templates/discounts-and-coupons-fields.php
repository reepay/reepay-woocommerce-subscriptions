...
<div class="show_if_reepay hidden">
	...
	if ( ! $is_update ): ?>
        <p class="form-field" style="display: none">
            <label for="use_existing_coupon"><?php
				_e( 'Coupon creation type', 'reepay-subscriptions-for-woocommerce' ); ?></label>
			<?php
			_e( 'Create new coupon', 'reepay-subscriptions-for-woocommerce' ); ?> &nbsp
            <input type="radio"
            ...
            &nbsp&nbsp
			<?php
			_e( 'Use existing coupon', 'reepay-subscriptions-for-woocommerce' ); ?> &nbsp
            <input
        ...
        <div class="show_if_use_existing_coupon">
            <p class="form-field" style="display: flex">
                <select name="_reepay_discount_use_existing_coupon_id" id="coupon_id" class="short">
                    <option value=""><?php
						_e( 'Select coupon', 'reepay-subscriptions-for-woocommerce' ); ?></option>
					<?php
					foreach ( $coupons as $coupon ): ?>
                        <option value="<?php
						echo esc_attr( $coupon['handle'] ) ?>"><?php
							echo esc_attr( $coupon['name'] ) ?></option>
					<?php
					endforeach; ?>
                </select>
                ...
                <a class="button button-primary button-large"
                   ...
                   href="https://admin.billwerk.plus/#/rp/config/coupons/create"
                   target="_blank">
					<?php
    ...
    <div class="hide_if_use_existing_coupon reepay_coupon_new">
		...
		if ( ! $is_update ): ?>
            <p class="form-field">
                <label for="use_existing_discount"><?php
					_e( 'Discount creation type', 'reepay-subscriptions-for-woocommerce' ); ?></label>
				<?php
				_e( 'Create new discount', 'reepay-subscriptions-for-woocommerce' ); ?> &nbsp
                <input type="radio"
                ...
                &nbsp&nbsp
				<?php
				_e( 'Use existing discount', 'reepay-subscriptions-for-woocommerce' ); ?> &nbsp
                <input
                ...
                <p class="form-field">
					...
					if ( ! empty( $discounts ) ): ?>
                        <select name="_reepay_discount_use_existing_discount_id" id="discount_id" class="short">
                            <option value=""><?php
								_e( 'Select discount', 'reepay-subscriptions-for-woocommerce' ); ?></option>
							<?php
							foreach ( $discounts as $discount ): ?>
                                <option value="<?php
								echo esc_attr( $discount['handle'] ) ?>"><?php
									echo esc_attr( $discount['name'] ) ?></option>
							<?php
							endforeach; ?>
                        </select>
					<?php
